...
export const Composer = forwardRef<ComposerRef, Props>(({dismiss, className, send, ...props}, ref) => {
    ...
    return (
        <form className={`flex flex-col gap-2 transition-transform-opacity ${className}`} onSubmit={submitEvent} { ...props }>
            <h2 className='text-xl font-normal text-center text-primary-100 sm:truncate sm:text-2xl sm:tracking-tight'>Log last night's dream</h2>
            <label htmlFor="dreamTitle">Title:</label>
            <input ref={dreamTitleInput} type="text" id="dreamTitle" className="w-full p-3 bg-primary-800 outline-primary-700 rounded-md" />
